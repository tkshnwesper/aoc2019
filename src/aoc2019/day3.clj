...
(defn- get-coordinate-range
  [magnitude]
  (if (nil? magnitude)
    '(0)
    (let [magnitude-neg? (neg? magnitude)]
      (range
       ((if magnitude-neg? - +) 1)
       ((if magnitude-neg? dec inc) magnitude)
       ((if magnitude-neg? - +) 1)))))

(defn get-point-range
  [last-point x-magnitude y-magnitude]
  (for [x (get-coordinate-range x-magnitude)
        y (get-coordinate-range y-magnitude)]
    (list
     (+ (first last-point) x)
     (+ (second last-point) y))))

(defn- get-point-range-fn
  [pointing]
  (case pointing
    U (fn [last-point magnitude] (get-point-range last-point nil magnitude))
    R (fn [last-point magnitude] (get-point-range last-point magnitude nil))
    L (fn [last-point magnitude] (get-point-range last-point (- magnitude) nil))
    D (fn [last-point magnitude] (get-point-range last-point nil (- magnitude)))))
...
(defn- get-points-
  ...
  (if (empty? wire-path)
    points
    (let [command (.toString (first wire-path))
          pointing (symbol (.toString (first command)))
          magnitude (Integer. (apply str (rest command)))
          last-point (get-last-point points)
